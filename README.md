...
这是一个为坦克设计游戏《Sprocket》编写的 MelonLoader 模组。它提高设计器中装甲测试工具的火炮口径与穿深上限，上限可在游戏内模组菜单的配置页中调整。

## 🛠️ 功能特性

* **突破限制**：解锁火炮关键参数的 UI 与底层上限，默认值如下，可在游戏内模组菜单的配置页中修改。
    * 穿深 (Penetration)：UI 滑块上限 1000mm（底层 20000mm）。
    * 口径 (Caliber)：UI 滑块范围 1mm - 500mm（底层 1mm - 5000mm）。
